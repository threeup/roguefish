...
public class PlayGamesPlatform : ISocialPlatform {
    ...
    public static bool DebugLogEnabled {
        get {
            return GoogLogger.DebugLogEnabled;
        }
        set {
            GoogLogger.DebugLogEnabled = value;
        }
    }
    ...
    public static PlayGamesPlatform Activate() {
        GoogLogger.d("Activating PlayGamesPlatform.");
        Social.Active = PlayGamesPlatform.Instance;
        GoogLogger.d("PlayGamesPlatform activated: " + Social.Active);
    ...
    public void Authenticate(Action<bool> callback, bool silent) {
        ...
        if (mClient == null) {
            GoogLogger.d("Creating platform-specific Play Games client.");
    ...
    public void LoadUsers(string[] userIDs, Action<IUserProfile[]> callback) {
        GoogLogger.w("PlayGamesPlatform.LoadUsers is not implemented.");
        if (callback != null) {
    ...
    public string GetUserId() {
        if (!IsAuthenticated()) {
            GoogLogger.e("GetUserId() can only be called after authentication.");
            return "0";
        }
    ...
    public string GetUserDisplayName() {
        if (!IsAuthenticated()) {
            GoogLogger.e("GetUserDisplayName can only be called after authentication.");
            return "";
        }
    ...
    public void ReportProgress(string achievementID, double progress, Action<bool> callback) {
        if (!IsAuthenticated()) {
            GoogLogger.e("ReportProgress can only be called after authentication.");
            if (callback != null) {
        ...
        GoogLogger.d("ReportProgress, " + achievementID + ", " + progress);
        achievementID = MapId(achievementID);

        // if progress is 0.0, we just want to reveal it
        if (progress < 0.000001) {
            GoogLogger.d("Progress 0.00 interpreted as request to reveal.");
        ...
        if (ach == null) {
            GoogLogger.w("Unable to locate achievement " + achievementID);
            GoogLogger.w("As a quick fix, assuming it's standard.");
            isIncremental = false;
        } else {
            isIncremental = ach.IsIncremental;
            curSteps = ach.CurrentSteps;
            totalSteps = ach.TotalSteps;
            GoogLogger.d("Achievement is " + (isIncremental ? "INCREMENTAL" : "STANDARD"));
            if (isIncremental) {
                GoogLogger.d("Current steps: " + curSteps + "/" + totalSteps);
            }
        }
        ...
        if (isIncremental) {
            ...
            GoogLogger.d("Progress " + progress +
            " interpreted as incremental target (approximate).");
            int targetSteps = (int)(progress * totalSteps);
            int numSteps = targetSteps - curSteps;
            GoogLogger.d("Target steps: " + targetSteps + ", cur steps:" + curSteps);
            GoogLogger.d("Steps to increment: " + numSteps);
            if (numSteps > 0) {
                mClient.IncrementAchievement(achievementID, numSteps, callback);
            }
        } else {
            // unlock it!
            GoogLogger.d("Progress " + progress + " interpreted as UNLOCK.");
    ...
    public void IncrementAchievement(string achievementID, int steps, Action<bool> callback) {
        if (!IsAuthenticated()) {
            GoogLogger.e("IncrementAchievement can only be called after authentication.");
            if (callback != null) {
                callback.Invoke(false);
            }
            return;
        ...
        // map ID, if it's in the dictionary
        GoogLogger.d("IncrementAchievement: " + achievementID + ", steps " + steps);
        achievementID = MapId(achievementID);
    ...
    public void LoadAchievementDescriptions(Action<IAchievementDescription[]> callback) {
        GoogLogger.w("PlayGamesPlatform.LoadAchievementDescriptions is not implemented.");
        if (callback != null) {
    ...
    public void LoadAchievements(Action<IAchievement[]> callback) {
        GoogLogger.w("PlayGamesPlatform.LoadAchievements is not implemented.");
        if (callback != null) {
    ...
    public void ReportScore(long score, string board, Action<bool> callback) {
        if (!IsAuthenticated()) {
            GoogLogger.e("ReportScore can only be called after authentication.");
            if (callback != null) {
                callback.Invoke(false);
            }
            return;
        ...
        GoogLogger.d("ReportScore: score=" + score + ", board=" + board);
    ...
    public void LoadScores(string leaderboardID, Action<IScore[]> callback) {
        GoogLogger.w("PlayGamesPlatform.LoadScores not implemented.");
        if (callback != null) {
    ...
    public ILeaderboard CreateLeaderboard() {
        GoogLogger.w("PlayGamesPlatform.CreateLeaderboard not implemented. Returning null.");
        return null;
    }
    ...
    public void ShowAchievementsUI() {
        if (!IsAuthenticated()) {
            GoogLogger.e("ShowAchievementsUI can only be called after authentication.");
            return;
        }

        GoogLogger.d("ShowAchievementsUI");
    ...
    public void ShowLeaderboardUI() {
        if (!IsAuthenticated()) {
            GoogLogger.e("ShowLeaderboardUI can only be called after authentication.");
            return;
        }
        GoogLogger.d("ShowLeaderboardUI");
    ...
    public void ShowLeaderboardUI(string lbId) {
        ...
        GoogLogger.d("ShowLeaderboardUI, lbId=" + lbId);
        if (lbId != null) {
            lbId = MapId(lbId);
        }
    ...
    public void SetDefaultLeaderboardForUI(string lbid) {
        GoogLogger.d("SetDefaultLeaderboardForUI: " + lbid);
        if (lbid != null) {
            lbid = MapId(lbid);
        }
        mDefaultLbUi = lbid;
    ...
    public void LoadFriends(ILocalUser user, Action<bool> callback) {
        GoogLogger.w("PlayGamesPlatform.LoadFriends not implemented.");
        if (callback != null) {
            callback.Invoke(false);
        }
    }
    ...
    public void LoadState(int slot, OnStateLoadedListener listener) {
        if (!IsAuthenticated()) {
            GoogLogger.e("LoadState can only be called after authentication.");
            if (listener != null) {
    ...
    public void UpdateState(int slot, byte[] data, OnStateLoadedListener listener) {
        if (!IsAuthenticated()) {
            GoogLogger.e("UpdateState can only be called after authentication.");
            if (listener != null) {
    ...
    private string MapId(string id) {
        ...
        if (mIdMap.ContainsKey(id)) {
            ...
            GoogLogger.d("Mapping alias " + id + " to ID " + result);
            return result;
        }
        return id;
    }
