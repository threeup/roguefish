...
public class NativeClient : IPlayGamesClient {
    ...
    private static void InvokeCallbackOnGameThread(Action<bool> callback, bool success) {
        ...
        PlayGamesHelperObject.RunOnGameThread(() => {
            GoogLogger.d("Invoking user callback on game thread");
            callback(success);
        });
    }
    ...
    internal void HandleInvitation(Types.MultiplayerEvent eventType, string invitationId,
        ...
        if (currentHandler == null) {
            GoogLogger.d("Received " + eventType + " for invitation "
            + invitationId + " but no handler was registered.");
            return;
        }

        if (eventType == Types.MultiplayerEvent.REMOVED) {
            GoogLogger.d("Ignoring REMOVED for invitation " + invitationId);
            return;
        }
    ...
    private void PopulateAchievements(uint authGeneration,
        ...
        if (authGeneration != mAuthGeneration) {
            GoogLogger.d("Received achievement callback after signout occurred, ignoring");
            return;
        }

        GoogLogger.d("Populating Achievements");
        lock (AuthStateLock) {
            if (response.Status() != Status.ResponseStatus.VALID &&
                response.Status() != Status.ResponseStatus.VALID_BUT_STALE) {
                GoogLogger.e("Error retrieving achievements - check the log for more information. " +
                    "Failing signin.");
        ...
        GoogLogger.d("Maybe finish for Achievements");
    ...
    void MaybeFinishAuthentication() {
        ...
        lock (AuthStateLock) {
            ...
            if (mUser == null || mAchievements == null) {
                GoogLogger.d("Auth not finished. User=" + mUser + " achievements=" + mAchievements);
                return;
            }

            GoogLogger.d("Auth finished. Proceeding.");
        ...
        if (localCallbacks != null) {
            GoogLogger.d("Invoking Callbacks: " + localCallbacks);
    ...
    void PopulateUser(uint authGeneration, PlayerManager.FetchSelfResponse response) {
        GoogLogger.d("Populating User");

        if (authGeneration != mAuthGeneration) {
            GoogLogger.d("Received user callback after signout occurred, ignoring");
            return;
        }
        ...
        lock (AuthStateLock) {
            if (response.Status() != Status.ResponseStatus.VALID &&
                response.Status() != Status.ResponseStatus.VALID_BUT_STALE) {
                GoogLogger.e("Error retrieving user, signing out");
        ...
        GoogLogger.d("Found User: " + mUser);
        GoogLogger.d("Maybe finish for User");
        MaybeFinishAuthentication();
    }

    private void HandleAuthTransition(Types.AuthOperation operation, Status.AuthStatus status) {
        GoogLogger.d("Starting Auth Transition. Op: " + operation + " status: " + status);
        lock (AuthStateLock) {
            switch (operation) {
                ...
                case Types.AuthOperation.SIGN_OUT:
                    ...
                    break;
                default:
                    GoogLogger.e("Unknown AuthOperation " + operation);
                    break;
            }
        }
    }
    ...
    private void UpdateAchievement(string updateType, string achId, Action<bool> callback,
        ...
        if (achievement == null) {
            GoogLogger.d("Could not " + updateType + ", no achievement with ID " + achId);
            callback(false);
            return;
        }

        if (alreadyDone(achievement)) {
            GoogLogger.d("Did not need to perform " + updateType + ": " + "on achievement " + achId);
            callback(true);
            return;
        }

        GoogLogger.d("Performing " + updateType + " on " + achId);
    ...
    public void IncrementAchievement(string achId, int steps, Action<bool> callback) {
        ...
        var achievement = GetAchievement(achId);
        if (achievement == null) {
            GoogLogger.e("Could not increment, no achievement with ID " + achId);
            callback(false);
            return;
        }

        if (!achievement.IsIncremental) {
            GoogLogger.e("Could not increment, achievement with ID " + achId + " was not incremental");
            callback(false);
            return;
        }

        if (steps < 0) {
            GoogLogger.e("Attempted to increment by negative steps");
            callback(false);
            return;
        }
    ...
    public void LoadState(int slot, OnStateLoadedListener listener) {
        ...
        lock (GameServicesLock) {
            if (mAuthState != AuthState.Authenticated) {
                GoogLogger.e("You can only call LoadState after the user has successfully logged in");
    ...
    public void UpdateState(int slot, byte[] data, OnStateLoadedListener listener) {
        ...
        lock (GameServicesLock) {
            if (mAuthState != AuthState.Authenticated) {
                GoogLogger.e("You can only call UpdateState after the user has successfully logged in");
