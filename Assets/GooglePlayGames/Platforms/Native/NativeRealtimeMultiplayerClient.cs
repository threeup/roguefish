...
public class NativeRealtimeMultiplayerClient : IRealTimeMultiplayerClient {
    ...
    public void CreateQuickGame(uint minOpponents, uint maxOpponents, uint variant,
        ...
        lock (mSessionLock) {
            ...
            if (mCurrentSession.IsActive()) {
                GoogLogger.e("Received attempt to create a new room without cleaning up the old one.");
            ...
            mRealtimeManager.ShowPlayerSelectUI(minOpponents, maxOppponents, true,
                response => {
                    if (response.Status() != Status.UIStatus.VALID) {
                        GoogLogger.d("User did not complete invitation screen.");
                        newRoom.LeaveRoom();
                        return;
                    }
    ...
    public void AcceptFromInbox(RealTimeMultiplayerListener listener) {
        lock (mSessionLock) {
            var newRoom = new RoomSession(mRealtimeManager, listener);
            if (mCurrentSession.IsActive()) {
                GoogLogger.e("Received attempt to accept invitation without cleaning up " +
                    "active session.");
                newRoom.LeaveRoom();
                return;
            }
            ...
            mRealtimeManager.ShowRoomInboxUI(
                response => {
                    ...
                    using (var helper = HelperForSession(newRoom)) {
                        GoogLogger.d("About to accept invitation " + invitation.Id());
                        newRoom.StartRoomCreation(mNativeClient.GetUserId(),
        ...
        lock (mSessionLock) {
            ...
            mRealtimeManager.FetchInvitations(response => {
                if (!response.RequestSucceeded()) {
                    GoogLogger.e("Couldn't load invitations.");
                    newRoom.LeaveRoom();
                    return;
                }
                ...
                GoogLogger.e("Room creation failed since we could not find invitation with ID "
                    + invitationId);
                newRoom.LeaveRoom();
            });
        }
    ...
    private class RoomSession {
        ...
        internal void EnterState(State handler) {
            lock (mLifecycleLock) {
                ...
                GoogLogger.d("Entering state: " + handler.GetType().Name);
        ...
        internal void StartRoomCreation(string currentPlayerId, Action createRoom) {
            lock (mLifecycleLock) {
                if (!mStillPreRoomCreation) {
                    GoogLogger.e("Room creation started more than once, this shouldn't happen!");
                    return;
                }

                if (!mState.IsActive()) {
                    GoogLogger.w("Received an attempt to create a room after the session was already " +
                        "torn down!");
                    return;
                }
    ...
    internal abstract class State {
        internal virtual void HandleRoomResponse(RealtimeManager.RealTimeRoomResponse response) {
            GoogLogger.d(this.GetType().Name + ".HandleRoomResponse: Defaulting to no-op.");
        }

        internal virtual bool IsActive() {
            GoogLogger.d(this.GetType().Name + ".IsNonPreemptable: Is preemptable by default.");
            return true;
        }

        internal virtual void LeaveRoom() {
            GoogLogger.d(this.GetType().Name + ".LeaveRoom: Defaulting to no-op.");
        }

        internal virtual void OnStateEntered() {
            GoogLogger.d(this.GetType().Name + ".OnStateEntered: Defaulting to no-op.");
        }

        internal virtual void OnRoomStatusChanged(NativeRealTimeRoom room) {
            GoogLogger.d(this.GetType().Name + ".OnRoomStatusChanged: Defaulting to no-op.");
        }

        internal virtual void OnConnectedSetChanged(NativeRealTimeRoom room) {
            GoogLogger.d(this.GetType().Name + ".OnConnectedSetChanged: Defaulting to no-op.");
        }

        internal virtual void OnParticipantStatusChanged(NativeRealTimeRoom room,
            MultiplayerParticipant participant) {
            GoogLogger.d(this.GetType().Name + ".OnParticipantStatusChanged: Defaulting to no-op.");
        }

        internal virtual void OnDataReceived(NativeRealTimeRoom room, MultiplayerParticipant sender,
            byte[] data, bool isReliable) {
            GoogLogger.d(this.GetType().Name + ".OnDataReceived: Defaulting to no-op.");
        }

        internal virtual void SendToSpecificRecipient(
            string recipientId, byte[] data, int offset, int length, bool isReliable) {
            GoogLogger.d(this.GetType().Name + ".SendToSpecificRecipient: Defaulting to no-op.");
        }

        internal virtual void SendToAll(byte[] data, int offset, int length, bool isReliable) {
            GoogLogger.d(this.GetType().Name + ".SendToApp: Defaulting to no-op.");
        }

        internal virtual List<Participant> GetConnectedParticipants() {
            GoogLogger.d(this.GetType().Name + ".GetConnectedParticipants: Returning empty connected" +
                " participants");
            return new List<Participant>();
        }

        internal virtual Participant GetSelf() {
            GoogLogger.d(this.GetType().Name + ".GetSelf: Returning null self.");
            return null;
        }

        internal virtual Participant GetParticipant(string participantId) {
            GoogLogger.d(this.GetType().Name + ".GetSelf: Returning null participant.");
            return null;
        }

        internal virtual bool IsRoomConnected() {
            GoogLogger.d(this.GetType().Name + ".IsRoomConnected: Returning room not connected.");
            return false;
        }
    }
    ...
    private abstract class MessagingEnabledState : State {
        ...
        internal override void SendToSpecificRecipient(
            ...
            if (!mNativeParticipants.ContainsKey(recipientId)) {
                GoogLogger.e("Attempted to send message to unknown participant " + recipientId);
                return;
            }
    ...
    class BeforeRoomCreateStartedState : State {
        ...
        internal override void LeaveRoom() {
            GoogLogger.d("Session was torn down before room was created.");
    ...
    class RoomCreationPendingState : State {
        ...
        internal override void LeaveRoom() {
            GoogLogger.d("Received request to leave room during room creation, aborting creation.");
    ...
    class ConnectingState : MessagingEnabledState {
        ...
        internal override void HandleConnectedSetChanged(NativeRealTimeRoom room) {
            ...
            if (mConnectedParticipants.Equals(newConnectedSet)) {
                GoogLogger.w("Received connected set callback with unchanged connected set!");
                return;
            }
            ...
            if (noLongerConnected.Any()) {
                GoogLogger.e("Participants disconnected during room setup, failing. " +
            ...
            GoogLogger.d("New participants connected: " +
            ...
            if (newConnectedSet.Count() == room.ParticipantCount()) {
                GoogLogger.d("Fully connected! Transitioning to active state.");
        ...
        internal override void HandleParticipantStatusChanged(NativeRealTimeRoom room,
            ...
            GoogLogger.e(string.Format("Participant {0} changed to status {1}, room will never be" +
    ...
    class ActiveState : MessagingEnabledState {
        ...
        internal override void OnStateEntered() {
            if (GetSelf() == null) {
                GoogLogger.e("Room reached active state with unknown participant for the player");
                LeaveRoom();
            }
        }
        ...
        internal override Participant GetParticipant(string participantId) {
            if (!mParticipants.ContainsKey(participantId)) {
                GoogLogger.e("Attempted to retrieve unknown participant " + participantId);
                return null;
            }
        ...
        internal override void HandleConnectedSetChanged(NativeRealTimeRoom room) {
            ...
            GoogLogger.d("Updated participant statuses: " +
            ...
            if (newlyLeft.Contains(GetSelf().ParticipantId)) {
                GoogLogger.e("Player was unexpectedly disconnected from the multiplayer session.");
                LeaveRoom();
                return;
            }
