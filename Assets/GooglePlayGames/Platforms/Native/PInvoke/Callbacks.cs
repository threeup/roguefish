...
static class Callbacks {
    ...
    internal static readonly Action<Status.UIStatus> NoopUICallback = status => {
        GoogLogger.d("Received UI callback: " + status);
    };
    ...
    private static T IntPtrToCallback<T>(IntPtr handle, bool unpinHandle) where T : class {
        ...
        try {
            ...
            GoogLogger.e("GC Handle pointed to unexpected type: " + gcHandle.Target.ToString() +
                ". Expected " +  typeof(T));
            throw e;
        } finally {
            if (unpinHandle) {
    ...
    internal static void InternalShowUICallback(Status.UIStatus status, IntPtr data) {
        GoogLogger.d("Showing UI Internal callback: " + status);
        var callback = IntPtrToTempCallback<Action<Status.UIStatus>>(data);

        try {
            callback(status);
        } catch (Exception e) {
            GoogLogger.e("Error encountered executing InternalShowAllUICallback. " +
    ...
    internal static void PerformInternalCallback(string callbackName, Type callbackType,
        ...
        GoogLogger.d("Entering internal callback for " + callbackName);
        ...
        try {
            ...
            GoogLogger.e("Error encountered executing " + callbackName + ". " +
            "Smothering to avoid passing exception into Native: " + e);
        }
    }
