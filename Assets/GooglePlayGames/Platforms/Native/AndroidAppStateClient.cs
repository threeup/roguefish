...
internal class AndroidAppStateClient : AppStateClient {
    ...
    public void LoadState(int slot, OnStateLoadedListener listener) {
        GoogLogger.d("LoadState, slot=" + slot);
    ...
    public void UpdateState(int slot, byte[] data, OnStateLoadedListener listener) {
        GoogLogger.d("UpdateState, slot=" + slot);
        using (var apiClient = GetApiClient(mServices)) {
    ...
    private class OnStateResultProxy : AndroidJavaProxy {
        ...
        private void OnStateConflict(int stateKey, string resolvedVersion,
            ...
            GoogLogger.d("OnStateResultProxy.onStateConflict called, stateKey=" + stateKey +
            ...
            if (mListener != null) {
                GoogLogger.d("OnStateResultProxy.onStateConflict invoking conflict callback.");
                ...
                GoogLogger.w("No conflict callback specified! Cannot resolve cloud save conflict.");
            }
        }

        private void ResolveState(int slot, string resolvedVersion, byte[] resolvedData,
            OnStateLoadedListener listener) {
            GoogLogger.d(string.Format("AndroidClient.ResolveState, slot={0}, ver={1}, " +
        ...
        private void OnStateLoaded(int statusCode, int stateKey, byte[] localData) {
            GoogLogger.d("OnStateResultProxy.onStateLoaded called, status " + statusCode +
                ", stateKey=" + stateKey);
            debugLogData("localData", localData);
            ...
            switch (statusCode) {
                case STATUS_OK:
                    GoogLogger.d("Status is OK, so success.");
                    success = true;
                    break;
                case STATUS_NO_DATA:
                    GoogLogger.d("Status is NO DATA (no network?), so it's a failure.");
                    success = false;
                    localData = null;
                    break;
                case STATUS_STALE_DATA:
                    GoogLogger.d("Status is STALE DATA, so considering as success.");
                    success = true;
                    break;
                case STATUS_KEY_NOT_FOUND:
                    GoogLogger.d("Status is KEY NOT FOUND, which is a success, but with no data.");
                    success = true;
                    localData = null;
                    break;
                default:
                    GoogLogger.e("Cloud load failed with status code " + statusCode);
                    success = false;
                    localData = null;
                    break;
            }
            ...
            if (mListener != null) {
                GoogLogger.d("OnStateResultProxy.onStateLoaded invoking load callback.");
                PlayGamesHelperObject.RunOnGameThread(() => {
                    mListener.OnStateLoaded(success, stateKey, localData);
                });
            } else {
                GoogLogger.w("No load callback specified!");
            }
        }

        private void debugLogData(string tag, byte[] data) {
            GoogLogger.d("   " + tag + ": " + GoogLogger.describe(data));
        }

        public void onResult(AndroidJavaObject result) {
            GoogLogger.d("OnStateResultProxy.onResult, result=" + result);

            int statusCode = GetStatusCode(result);
            GoogLogger.d("OnStateResultProxy: status code is " + statusCode);

            if (result == null) {
                GoogLogger.e("OnStateResultProxy: result is null.");
                return;
            }

            GoogLogger.d("OnstateResultProxy: retrieving result objects...");
            ...
            GoogLogger.d("Got result objects.");
            GoogLogger.d("loadedResult = " + loadedResult);
            GoogLogger.d("conflictResult = " + conflictResult);

            if (conflictResult != null) {
                GoogLogger.d("OnStateResultProxy: processing conflict.");
                ...
                GoogLogger.d("OnStateResultProxy: conflict args parsed, calling.");
                OnStateConflict(stateKey, ver, localData, serverData);
            } else if (loadedResult != null) {
                GoogLogger.d("OnStateResultProxy: processing normal load.");
                int stateKey = loadedResult.Call<int>("getStateKey");
                byte[] localData = ToByteArray(loadedResult.NullSafeCall("getLocalData"));
                GoogLogger.d("OnStateResultProxy: loaded args parsed, calling.");
                OnStateLoaded(statusCode, stateKey, localData);
            } else {
                GoogLogger.e("OnStateResultProxy: both loadedResult and conflictResult are null!");
            }
        }
    }
